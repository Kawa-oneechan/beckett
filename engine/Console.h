...
namespace Beck
{
	...
	//part of the tree.
	class Console : public Beck::Tickable
	{
	private:
...
#define conprint(C, F, ...) Beck::console->Print(C, fmt::format(F, __VA_ARGS__))
#ifdef DEBUG
#define debprint(C, F, ...) Beck::console->Print(C, fmt::format(F, __VA_ARGS__))
#else
#define debprint(C, F, ...)
#endif
