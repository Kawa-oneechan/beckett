...
void main()
{
    vec2 pos = (2. * gl_FragCoord.xy - ScreenRes.xy ) / min(ScreenRes.x, ScreenRes.y) + vec2(.0, .4);
    //vec4 col = vec4(.5);
    vec4 col = vec4(0.0);
    float x = tri(TotalTime * .1) * 2.6;
    vec2 s = 2. * pos - vec2(x, 1.5 * abs(sin(TotalTime * 1.7)));
    ...
    {
        //if (grid(pos, -.3, 1.205)) { col.g = 0.25; col.a = 1.0; }
        pos = vec2(pos.x, 1.) / pos.y * -.3;
        //if (grid(pos, .7, 1.)) col.g = 0.5;
        if (length(s - vec2(.4, .1)) < .85) col = vec4(0.0, 0.0, 0.0, 0.25);
    }
